...
int verify_upper(uint32_t *a, uint32_t *b, int n){
  ...
  errors = 0 ;
  for(i=0 ; i<n ; i++) errors += ((a[i]^b[i]) & mask) ? 1 : 0 ;
  return errors ;
}
...
int main(int argc, char **argv){
  ...
  errors = verify_upper(fp32, fp32b, n) ;
  printf("restored  : ");
  for(i=0 ; i<8 ; i++) printf("%8.8x ",fp32b[i]) ; printf(" errors = %d\n", errors) ;
  tmin = 1000000000 ;  nt = 0 ; tavg = 0.0f ; tmax = 0 ;
  for(j=0 ; j < NTIMES ; j++) {
    t0 = elapsed_cycles() ;
    bf24_fp32(lfp32b, luf24, NPTS) ;
    t1 = elapsed_cycles() ;
    t[j] = t1 - t0 ;
    tmin = (t[j] < tmin) ? t[j] : tmin ;
  }
  ...
  memset(fp32b, 0xFF, sizeof(fp32b));
  u24u32(fp32b, uf24, n) ;
  errors = verify_lower(fp32, fp32b, n) ;
  printf("restored  : ");
  for(i=0 ; i<8 ; i++) printf("%8.8x ",fp32b[i]) ; printf(" errors = %d\n", errors) ;
  tmin = 1000000000 ;  nt = 0 ; tavg = 0.0f ; tmax = 0 ;
  for(j=0 ; j < NTIMES ; j++) {
